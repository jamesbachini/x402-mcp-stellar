...
import { Backstop, BackstopPoolUser, BackstopPoolV1, BackstopPoolV2, getOracleDecimals, PoolMetadata, PoolV1, PoolV2, TokenMetadata, Version, poolEventV1FromEventResponse, poolEventV2FromEventResponse, RequestType, PoolFactoryContractV2, parseError, parseResult, PoolContractV2, } from '@blend-capital/blend-sdk';
import stellarSdk from '@stellar/stellar-sdk';
import { xdr, Keypair, Transaction, Address, } from '@stellar/stellar-sdk';
...
export class BlendService {
    ...
    async loadPoolMeta(poolId) {
        ...
        throw new Error(`Could not determine pool version for ${poolId}. Unknown wasmHash: ${metadata.wasmHash}`);
    }
    // Pool (V1/V2)
    async loadPool(poolId, meta) {
        if (!poolId)
            throw new Error('poolId is required');
        const network = getNetwork();
        if (meta) {
    ...
    // Pool User
    async loadPoolUser(pool, userAddress) {
        if (!pool || !userAddress)
            throw new Error('pool and userAddress are required');
        return await pool.loadUser(userAddress);
    }
    // Pool Oracle
    async loadPoolOracle(pool) {
        const network = getNetwork();
    ...
    // Pool Events
    async getPoolEvents(poolId, version, startLedger) {
    ...
    async loadBackstopPool(poolMeta) {
        ...
        if (poolMeta.version === Version.V2) {
            if (!process.env.BACKSTOP_ID_V2)
                throw new Error('BACKSTOP_ID_V2 env variable is required');
            return await BackstopPoolV2.load(network, process.env.BACKSTOP_ID_V2, poolMeta.id);
        }
        else {
            if (!process.env.BACKSTOP_ID)
                throw new Error('BACKSTOP_ID env variable is required');
            return await BackstopPoolV1.load(network, process.env.BACKSTOP_ID, poolMeta.id);
        }
    }
    // Backstop Pool User
    async loadBackstopPoolUser(poolMeta, userAddress) {
        ...
        if (poolMeta.version === Version.V2) {
            if (!process.env.BACKSTOP_ID_V2)
                throw new Error('BACKSTOP_ID_V2 env variable is required');
            return await BackstopPoolUser.load(network, process.env.BACKSTOP_ID_V2, poolMeta.id, userAddress);
        }
        else {
            if (!process.env.BACKSTOP_ID)
                throw new Error('BACKSTOP_ID env variable is required');
            return await BackstopPoolUser.load(network, process.env.BACKSTOP_ID, poolMeta.id, userAddress);
        }
    }
    // Token Metadata
    async loadTokenMetadata(assetId) {
        if (!assetId)
    ...
    async getTokenBalance(tokenId, userAddress) {
        if (!tokenId || !userAddress)
            throw new Error('tokenId and userAddress are required');
        const network = getNetwork();
        try {
    ...
    async lend({ userAddress, amount, asset, poolId, privateKey }) {
        ...
        const txHash = await this._invokeSorobanOperation(supplyOp, () => '', // No parser needed, just return the hash
        txParams);
        return txHash;
    }
    async withdraw({ userAddress, amount, asset, poolId, privateKey }) {
        if (!userAddress || !amount || !asset || !poolId)
            throw new Error('userAddress, amount, asset, and poolId are required');
        const network = getNetwork();
        const pool = new PoolContractV2(poolId);
        ...
        // Call _invokeSorobanOperation
        const txHash = await this._invokeSorobanOperation(withdrawOp, () => '', // No parser needed, just return the hash
        txParams);
        return txHash;
    }
    async borrow({ userAddress, amount, asset, poolId, privateKey }) {
        if (!userAddress || !amount || !asset || !poolId)
            throw new Error('userAddress, amount, asset, and poolId are required');
        const network = getNetwork();
        const pool = new PoolContractV2(poolId);
        ...
        // Call _invokeSorobanOperation
        const txHash = await this._invokeSorobanOperation(borrowOp, () => '', // No parser needed, just return the hash
        txParams);
        return txHash;
    }
    async repay({ userAddress, amount, asset, poolId, privateKey }) {
        if (!userAddress || !amount || !asset || !poolId)
            throw new Error('userAddress, amount, asset, and poolId are required');
        const network = getNetwork();
        const pool = new PoolContractV2(poolId);
        ...
        // Call _invokeSorobanOperation
        const txHash = await this._invokeSorobanOperation(repayOp, () => '', // No parser needed, just return the hash
        txParams);
        return txHash;
    }
    async claim({ userAddress, poolId, reserveTokenIds, privateKey }) {
        if (!userAddress || !poolId || !reserveTokenIds)
            throw new Error('userAddress, poolId, and reserveTokenIds are required');
        const network = getNetwork();
        const pool = new PoolContractV2(poolId);
        ...
        // Call _invokeSorobanOperation
        const txHash = await this._invokeSorobanOperation(claimOp, () => '', // No parser needed, just return the hash
        txParams);
        return txHash;
    }
    async createPool({ admin, name, oracleId, backstopRate, maxPositions, minCollateral, privateKey }) {
        ...
        try {
            ...
            const poolAddress = await this._invokeSorobanOperation(operation, PoolFactoryContractV2.parsers.deployPool, txParams);
            if (!poolAddress) {
    ...
    async addReserve({ admin, poolId, assetId, metadata, privateKey }) {
        ...
        // Queue the reserve
        await this._invokeSorobanOperation(pool.queueSetReserve(setReserveArgs), PoolContractV2.parsers.queueSetReserve, txParams);
        // Try to set the reserve (may fail if time-lock not reached)
        let setResult = '';
        try {
            await this._invokeSorobanOperation(pool.setReserve(assetId), PoolContractV2.parsers.setReserve, txParams);
    ...
    // Simulate Operation
    async simulateOperation(operationXdr, userAddress) {
        if (!operationXdr || !userAddress)
            throw new Error('operationXdr and userAddress are required');
        const network = getNetwork();
     ...
     */
    async buyNft({ userAddress, nftContractId, tokenId, price, privateKey }) {
        ...
        // Call _invokeSorobanOperation
        const txHash = await this._invokeSorobanOperation(op.toXDR('base64'), () => '', txParams);
        return txHash;
    }
    // --- Internal helper for tx build/sign/submit ---
    async _submitTx(source, operations, privateKey, network) {
        if (!source || !operations || !network)
            throw new Error('source, operations, and network are required');
        const keypair = getKeypair(privateKey);
        if (source !== keypair.publicKey()) {
            throw new Error(`Transaction source address (${source}) does not match signing keypair public key (${keypair.publicKey()}). The privateKey for the source account must be provided.`);
        }
        const stellarRpc = new stellarSdk.rpc.Server(network.rpc, network.opts);
    ...
    async _sendTransaction(transaction, parser) {
        const network = getNetwork();
        const stellarRpc = new stellarSdk.rpc.Server(network.rpc, network.opts);
        try {
            // Submit transaction
            ...
            if (sendResponse.status !== 'PENDING') {
                ...
                if (sendResponse.diagnosticEvents) {
                    ...
                    sendResponse.diagnosticEvents.forEach((event, index) => {
    ...
    async _invokeSorobanOperation(operation, parser, txParams) {
        const network = getNetwork();
        const stellarRpc = new stellarSdk.rpc.Server(network.rpc, network.opts);
        try {
            ...
            if (stellarSdk.rpc.Api.isSimulationRestore(simulation)) {
                ...
                const restoreSignedTx = new stellarSdk.Transaction(await txParams.signerFunction(restoreTx.toXDR()), network.passphrase);
            ...
            const signedTx = new stellarSdk.Transaction(await txParams.signerFunction(assembledTx.toXDR()), network.passphrase);
